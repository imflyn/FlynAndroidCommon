...
public class MemoryCache
{
     ...
     */
    private static final int                                   SOFT_CACHE_SIZE = 10;  // 软引用缓存容量
    public static LruCache<String, Bitmap>                     mLruCache;             // 硬引用缓存
    public static LinkedHashMap<String, SoftReference<Bitmap>> mSoftCache;            // 软引用缓存
    ...
    private MemoryCache(Context context)
    {
        ...
        int cacheSize = 1024 * 1024 * 10; // 硬引用缓存容量，为系统可用内存的1/8
    ...
    {
        System.out.println("mSoftCache.clear()");
        mSoftCache.clear();
    }
}
