...
public class IntentUtils
{
    ...
    /**
     * Open app page at Google Play. If Play Store application isn't available
     * on the device then web browser will be opened
     *
     ...
     *
     * @param context       Application context
     * @param openInBrowser Should we try to open application page in web browser if Play
     *                      Store app not found on device
     */
    ...
    /**
     * Opens the Street View application to the given location. The URI scheme
     * is based on the syntax used for Street View panorama information in
     * Google Maps URLs.
     *
     ...
     * @param pitch     Panorama center-of-view in degrees from -90 (look straight up)
     *                  to 90 (look straight down.)
     * @param zoom      Panorama zoom. 1.0 = normal zoom, 2.0 = zoomed in 2x, 3.0 =
     *                  zoomed in 4x, and so on. A zoom of 1.0 is 90 degree horizontal
     *                  FOV for a nominal landscape mode 4 x 3 aspect ratio display
     *                  Android phones in portrait mode will adjust the zoom so that
     *                  the vertical FOV is approximately the same as the landscape
     *                  vertical FOV. This means that the horizontal FOV of an Android
     *                  phone in portrait mode is much narrower than in landscape
     *                  mode. This is done to minimize the fisheye lens effect that
     *                  would be present if a 90 degree horizontal FOV was used in
     *                  portrait mode.
     * @param mapZoom   The map zoom of the map location associated with this
     *                  panorama. This value is passed on to the Maps activity when
     *                  the Street View "Go to Maps" menu item is chosen. It
     *                  corresponds to the zoomLevel parameter in
     *                  {@link #showLocation(float, float, Integer)}
     */
    public static Intent showStreetView(float latitude, float longitude, Float yaw, Integer pitch, Float zoom, Integer mapZoom)
    {
     ...
     * @param latitude  Latitude
     * @param longitude Longitude
     * @param zoomLevel A zoom level of 1 shows the whole Earth, centered at the given
     *                  lat,lng. A zoom level of 2 shows a quarter of the Earth, and
     *                  so on. The highest zoom level is 23. A larger zoom level will
     *                  be clamped to 23.
     ...
     * Intent in onActivityResult. See code below for example:
     * <p/>
     * <p/>
     * <pre>
     * <code>
     ...
     * IETF RFC 3966 are accepted. Valid examples include the following:
     ...
     * Telephone number normalization described for {@link #callPhone(String)}
     * applies to dial as well.
     *
     * @param phoneNumber Phone number
     */
     ...
     * @see #cropImage(android.content.Context, java.io.File, int, int, int,
     * int, boolean)
     */
    ...
    /**
     * Crop image. Before using, cropImage requires especial check that differs
     * from
     ...
     *
     * @param context Application context
     * @param image   Image that will be used for cropping. This image is not
     *                changed during the cropImage
     ...
     * @param scale   Scale or not cropped image if output image and cropImage frame
     *                sizes differs
     * @return Intent with <code>data</code>-extra in
     * <code>onActivityResult</code> which contains result as a
